...
void check_cuda(cudaError_t status) {
    if (status != cudaSuccess) {
        std::cout << "CUDA API failed with error: " << cudaGetErrorString(status) << std::endl;
...
void check_cusparse(cusparseStatus_t status) {
    if (status != CUSPARSE_STATUS_SUCCESS) {
        std::cout << "CUSPARSE API failed with error: " << cusparseGetErrorString(status) << std::endl;
...
void check_cudnn(cudnnStatus_t status) {
    if (status != CUDNN_STATUS_SUCCESS) {
        std::cout << "CUDNN API failed with error: " << cudnnGetErrorString(status) << std::endl;
...
void check_cublas(cublasStatus_t status) {
    if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "CUBLAS API failed with error: " << cublasGetErrorString(status) << std::endl;
